...
<?php
    ...
    //########## CREAR UNA TABLA DE "BARRIOS" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    ...
                    idNomenclatura int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    PRIMARY KEY(id),				
                    FOREIGN KEY(idMunicipio) REFERENCES municipios (id),
                    FOREIGN KEY(idNomenclatura) REFERENCES nomenclaturas (id)
                )
            ';
        //Ejecutar
        crearTabla();
        ...
        $barrios = array(
            ...
            array(40,2,"La Lejana"),
            array(40,1,"Centro"),
            array(40,1,"El Porvenir"),
            );       
    ...
    //########## CREAR UNA TABLA DE "SEDES" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    id int NOT NULL AUTO_INCREMENT,
                    idInstEducativa int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    dane int(10),
                    PRIMARY KEY(id),				
                    FOREIGN KEY(idInstEducativa) REFERENCES instEducativas (id)
                )
            ';
        //Ejecutar
        crearTabla();
        ...
        $sedes = array(
            ...
            array(1,"Vereda Cercana"),
            array(1,"Vereda Lejana"),
            );        
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    ...
                    idJornada int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci,
                    PRIMARY KEY(id),
    ...
    //########## CREAR UNA TABLA DE "GRADOS POR JORNADA" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    id int NOT NULL AUTO_INCREMENT,
                    idJornadaSede int(2) NOT NULL,
                    idGrado int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci,
                    PRIMARY KEY(id),
                    FOREIGN KEY(idJornadaSede) REFERENCES jornadasXsede (id),				
    ...
    //########## CREAR UNA TABLA DE "GRUPOS POR GRADO" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    ...
                    idGradoJornada int(2) NOT NULL,
                    idGrupo int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    PRIMARY KEY(id),				
                    FOREIGN KEY(idGrupo) REFERENCES grupos (id),
                    FOREIGN KEY(idGradoJornada) REFERENCES gradosXjornada (id)
                )
            ';
        //Ejecutar
        crearTabla();
        ...
        $gruposXgrado = array(
            ...
            array(8,8,"Séptimo 1"),
            array(9,9,"Octavo 1"),
            array(10,10,"Noveno 1"),
            array(11,11,"Décimo 1"),
            array(12,12,"Once 1"),
        );        
    ...
    //########## CREAR UNA TABLA DE "ÁREAS POR GRUPO" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    id int NOT NULL AUTO_INCREMENT,
                    idGrupoGrado int(2) NOT NULL,
                    idArea int(2) NOT NULL,
                    nombre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    PRIMARY KEY(id),				
                    FOREIGN KEY(idArea) REFERENCES areas (id),
                    FOREIGN KEY(idGrupoGrado) REFERENCES gruposXgrado (id)
                )
            ';
        //Ejecutar
        crearTabla();
        ...
        $j=0;
        for ($i=1;$i<=12;$i++){
                $areasXgrupo[$j]=array($i,1,'Matemáticas');
                $areasXgrupo[$j+1]=array($i,2,'Ciencias');
                $areasXgrupo[$j+2]=array($i,3,'Lenguaje');
                $areasXgrupo[$j+3]=array($i,4,'Convivencia');
                $areasXgrupo[$j+4]=array($i,5,'Socialización');
                $areasXgrupo[$j+5]=array($i,6,'Participación');
                $areasXgrupo[$j+6]=array($i,7,'Autonomía');
                $areasXgrupo[$j+7]=array($i,8,'Autocontrol');
                $j=$j+8;
        }
    ...
    //########## CREAR UNA TABLA DE "PERIODOS POR ÁREA" ##########
        ...
        $k=0;
        for ($i=1;$i<=$j-1;$i++){
                $periodosXarea[$k]=array($i,1,'');
                $periodosXarea[$k+1]=array($i,2,'');
                $periodosXarea[$k+2]=array($i,3,'');
                $periodosXarea[$k+3]=array($i,4,'');
                $periodosXarea[$k+4]=array($i,5,'');
                $k=$k+5;
        }
    ...
    //########## CREAR UNA TABLA DE "MADRES" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    id int NOT NULL AUTO_INCREMENT,
                    nomMadre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    docMadre int(10) NOT NULL UNIQUE,
    ...
    //########## CREAR UNA TABLA DE "PADRES" ##########
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    id int NOT NULL AUTO_INCREMENT,
                    nomPadre varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL,
                    docPadre int(10) NOT NULL UNIQUE,
                    idOcupacion int(2) NOT NULL,
                    idNivelEdu int(2) NOT NULL,                
                    PRIMARY KEY(id),
                    FOREIGN KEY(idOcupacion) REFERENCES ocupaciones (id),
        ...
        $sql=
            '
                CREATE TABLE IF NOT EXISTS '.$tabla.'(
                    ...
                    idDepartamentoResidencia int(2) NOT NULL,
                    idMunicipioResidencia int(2) NOT NULL,
                    direccion varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci,
                    idBarrio int(5) NOT NULL,
                    telefono int(11) NOT NULL,
                    correo varchar(80) CHARACTER SET utf8 COLLATE utf8_spanish_ci,
                    idOpcProteccion int(2) NOT NULL,
                    idCentroProteccion int(2) NOT NULL,
                    idAspiraAlGrado int(2) NOT NULL,
                    idOpcEtnico int(2) NOT NULL,
                    ...
                    FOREIGN KEY(idDepartamentoResidencia) REFERENCES departamentos (id),
                    FOREIGN KEY(idMunicipioResidencia) REFERENCES municipios (id),
                    FOREIGN KEY(idBarrio) REFERENCES barrios (id),
                    FOREIGN KEY(idOpcProteccion) REFERENCES opciones (id),
                    FOREIGN KEY(idCentroProteccion) REFERENCES centrosDEproteccion (id),
                    FOREIGN KEY(idAspiraAlGrado) REFERENCES grados (id),
                    FOREIGN KEY(idOpcEtnico) REFERENCES opciones (id),
